...
def to_dic_head_col_index(sheet, head_col: str = "A"): #输出表头的index字典（从0开始）
    index_lib_0 = {}
    for index, cell in enumerate(sheet[head_col]):
        if cell.value is not None and cell.value != '':
            index_lib_0[cell.value] = index
    return index_lib_0

def to_dic_head_col_row(sheet, head_col: str = "A"): #输出表头的index字典（从0开始）
    index_lib_0 = {}
    for index, cell in enumerate(sheet[head_col]):
        if cell.value is not None and cell.value != '':
            index_lib_0[cell.value] = index + 1
    return index_lib_0