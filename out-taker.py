...
sys.path.append(r"/Users/edison/PycharmProjects/PythonProject")  # 这里的r这里的 r 是 Python 里的原始字符串标记，作用是让字符串里的特殊字符（比如\、空格等）“原样保留”，不会被 Python 解析成转义符。# 举个例子：如果直接写路径 "C:\Users\test"，Python 会把 \U 当成转义符，导致报错；但加了 r 写成 r"C:\Users\test"，Python 就会把整个字符串当成 “原始内容”，不会解析转义，路径就能正确识别。
...
def out_taker(file_path: str,
    ...
    if not target_row:
        return 'No cfg found'
    elif ws.cell(target_row, head_col_dic['Input Qty']).value >= quantity: # 如果目标cfg数量大于要拿的量，就开始拿出来（减去拿出来的数量）并标记红色
